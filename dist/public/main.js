"use strict";{
    ...
    const exts = cfg.icons?.map(x => x.ext.split('|')).flat().concat(cfg.systemExt.split('|').filter(Boolean))
    const individual = cfg.systemIndividual.split('|').filter(Boolean)
    ...
    HFS.onEvent('entryIcon', ({ entry: { ext, isFolder, uri } }, tools, output) => {
        ...
        if (!isFolder && individual?.includes(ext))
